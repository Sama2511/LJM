...
import { CreateEvent, FetchEventForEdit, UpdateEvent } from "@/actions/events";
import {
  Sheet,
  SheetTrigger,
  SheetContent,
...
export default function EditEventForm(id: { id: string }) {
  ...
  const handleEdit = async () => {
    setIsLoading(true);
    const supabase = createClient();
    const eventData = await FetchEventForEdit(id.id);
    if (eventData.data) {
      const { data: roles } = await supabase
        .from("event_roles")
        .select("role_name, capacity")
        .eq("event_id", id.id);

      setSelectedEvent({
        ...eventData.data,
        roles:
          roles && roles.length > 0 ? roles : [{ role_name: "", capacity: 1 }],
      });
      setImageUrl([
        `https://ogvimirljuiaxibowzul.supabase.co/storage/v1/object/public/event-pics/${eventData.data.image_url}`,
      ]);
    }
    setIsLoading(false);
  };
